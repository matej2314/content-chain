...
export const isRunStatus = (value: string): value is RunStatus => (RUN_STATUSES as readonly string[]).includes(value);
export const isRunTaskType = (value: string): value is RunTaskType => (RUN_TASK_TYPES as readonly string[]).includes(value);
export const isSocialPlatform = (value: string): value is SocialPlatform => (SOCIAL_PLATFORMS as readonly string[]).includes(value);
export const isContentLanguage = (value: string): value is ContentLanguage => (CONTENT_LANGUAGES as readonly string[]).includes(value);
