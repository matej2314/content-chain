...
| `packages/shared`          | Lekkie, współdzielone typy publicznego kontraktu API (bez logiki biznesowej)                                |
...
- **Strategiczne DDD „lekko”:** bounded contexty i wspólny język (`CompanyContext`, `Run`, `PostIdeas`, `PostContent`, …) — **bez** narzucania pełnej taktyki DDD (bogate agregaty / event sourcing wszędzie) w v1.
...
| ------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| **Auth** | Użytkownicy, role `admin` \| `user`, sesja JWT + httpOnly cookie | **Jeden** `admin` (bootstrap); tylko on edytuje kontekst; obaj mogą uruchamiać flow’y SM — `security.md` |
...
Rozszerzenia o kolejnych agentów (poczta, dokumenty, rolki itd.) — **poza MVP**; architektura zakłada dodawanie kolejnych contextów / grafów bez rozbijania monorepo na mikroserwisy.
...
- **Ports / adapters:** Prisma + SQLite jako domyślny adapter persistence (ORM tylko w infrastructure); klient HTTP (lub równoważny) do gateway jako adapter portu LLM.

**Anty-patterny do unikania:** reguły biznesowe w controllerze; bezpośrednie wywołania vendorów LLM z `apps/api`; logika SM w `apps/frontend` lub w gateway; synchroniczne blokowanie HTTP na cały długi run LLM.
...
3. Live postęp (status, logi przyrostowe, sygnał HITL, completed/failed) idzie do klienta przez **SSE** (`GET /api/v1/runs/:runId/events`). **GET** równolegle: snapshot logów runu oraz `GET /api/v1/health` — bez pollingu statusu jako kanału live.
...
Zmiana względem wcześniejszego zapisu w tym dokumencie: zamiast opierania obserwacji runu na samym pollingu HTTP — **SSE od v1** (szczegóły kontraktu: `dokumentacja_komunikacji.md`).

## Auth

- Mechanizm: **JWT** (access) + **refresh w httpOnly cookie** dla `apps/frontend`.
- Role: `admin`, `user` — zgodnie z dokumentacją koncepcyjną.
- Poza MVP auth: OAuth / social login, rozbudowane ABAC.

## Persistence

- Port persistence + adapter **Prisma / SQLite** jako domyślny self-host MVP.
- Opcjonalny adapter PostgreSQL — później, gdy pojawi się potrzeba deploymentu; nie wymóg v1.
...
| --------------- | ----------------------------------------------- | ---------------------------------------------- |
| Forma systemu   | Modularny monolit, 3 app + shared               | Mikroserwisy domenowe w v1                     |
...
| Auth            | JWT + httpOnly cookie, 2 role                   | OAuth w v1                                     |
| DB              | Prisma + SQLite first via port                  | Postgres jako wymóg MVP                        |
| Layout monorepo | `apps/*` + `packages/shared` w rootcie          | Rootowy `src/apps/` (porzucone)                |
| Shared          | `packages/shared` typy kontraktu                | Duplikacja DTO bez wspólnego źródła            |
