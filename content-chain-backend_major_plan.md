...
**Refaktor względem:** **Faza 6 / Krok 6.3** (`WYKONANY`) — `POST .../output-edited` ustawiało flagę, **bez** nadpisu payloadu wyniku; **Faza 5 / Krok 5.1** (`WYKONANY`) — probe `GET /auth/me` bez mutacji własnego emaila; **Faza 3 / Krok 3.3** (`WYKONANY`) — `GET /runs` filtr `status` jako **jeden** enum. MILESTONE 5 i 6 pozostają `OSIĄGNIĘTY` (historia bez zmian).

**Opis:** Trzy zmiany kontraktu `apps/api`, żeby major frontendowy wołał kanon z `docs/dokumentacja_komunikacji.md` / `docs/ux_dashboard.md` / `SPEC-RUNY.md` R-10 / R-3a / `SPEC-AUTH.md` A-3b / `SPEC-TESTY.md` D-12, D-27 i D-28. Pipeline / verifier / graf **nie** startują przy zapisie edycji. `PATCH /users/:id` nadal bez `email` (reaktywacja). Brak nowego endpointu SSE (FE = N× istniejące `.../events`).

**Bez MILESTONE 10** — korekta istniejącego kontraktu pod FE, nie nowy skok produktowy backendu. Faza 9 (`WYKONANY`) bez zmian treści.
...
- Zalogowany zmienia własny email; zajęty adres → konflikt; `GET /auth/me` zgadza się z nowym adresem.
- `PATCH /users/:id` z polem email nadal odrzucony.
- `GET /runs?status=completed,failed` zwraca wyłącznie te statusy (strona 10, `createdAt` desc); pojedynczy `status=` bez regresji; zła wartość → 400.
- Istniejący happy path oceny / finalize / sesji / listingu z Fazy 3–6 bez regresji.
...
**Opis:** `POST /api/v1/runs/:runId/output-edited` przyjmuje `{ result }` w kształcie snapshotu właściwym dla `taskType`. Kanoniczny wynik w DB zostaje zastąpiony; `outputEdited: true`; `characterCount` z długości `body` po stronie api. Ta sama kardynalność i te same `sourceIdeaId` / `id` pozycji. Authz / status / lock jak ocena (`completed` \| `failed`, autor, nie po finalize). Wielokrotny zapis do finalize. Zgodnie z `docs/dokumentacja_komunikacji.md`, `SPEC-RUNY.md` R-10, `SPEC-TESTY.md` D-12.

**DoD (krok):**

- Body z `result` jest obowiązkowe; puste / obcy kanał / zmiana id pozycji → **400**.
- GET snapshot po zapisie zwraca treść użytkownika jako wynik; flaga `outputEdited: true`.
- Pipeline / verifier / graf Social i Content nie startują.
- Te same **403** / **409** `REVIEW_LOCKED` / `RUN_NOT_REVIEWABLE` co przy ocenie.
- Case D-12 przechodzi (nadpis, nie sama flaga).
...
**Opis:** `PATCH /api/v1/auth/me` `{ email }` na sesji zalogowanego (admin i `user`). **200** `{ id, email, role }`. Zajęty `User.email` (w tym soft-deleted) → **409** `CONFLICT`, porównanie case-sensitive jak zaproszenia. Nie `PATCH /users/:id`. Bez zmiany hasła / roli / `isActive`. Bez maila potwierdzającego w MVP. Zgodnie z `docs/dokumentacja_komunikacji.md`, `SPEC-AUTH.md` A-3b, `SPEC-TESTY.md` D-27.

**DoD (krok):**

- Sesja: **200** z nowym emailem; kolejny `GET /auth/me` zwraca ten sam adres.
- Kolizja emaila → **409**; zły kształt → **400**; brak sesji → **401**.
- `PATCH /users/:id` z `email` nadal **400**.
...
**Opis:** `GET /api/v1/runs` przyjmuje `status` = jeden `RunStatus` **albo** lista unikalnych wartości rozdzielona przecinkiem (np. `completed,failed` pod archiwum UI). Sort `createdAt` desc i `pageSize=10` bez zmian. Brak parametru = wszystkie statusy (Postman / ops). Nieznana wartość → **400** `VALIDATION_FAILED`. Powtórki w liście = zbiór. Zgodnie z `docs/dokumentacja_komunikacji.md`, `SPEC-RUNY.md` R-3a, `SPEC-TESTY.md` D-28, `docs/ux_dashboard.md`.

**DoD (krok):**

- `status=completed,failed` → wyłącznie te dwa statusy, najnowsze pierwsze, strona 10.
- `status=interrupted` (pojedynczy) bez regresji względem Fazy 3.
- Brak `status` → pełna instancja jak dotąd.
- Zła wartość w liście → **400**.
