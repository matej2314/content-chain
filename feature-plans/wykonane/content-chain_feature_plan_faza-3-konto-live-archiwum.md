...
### KROK 4.1 — Refaktor szczegółów Run: identyczność snapshotu z `runId`

**Status:** `WYKONANY`

Refaktor względem: FAZA 2 / KROK 4 (WYKONANY). Plik `run-details-view.tsx` (oraz `key` na stronie z KROK 4). Cel: przy client-side zmianie `runId` nie otwierać SSE na podstawie **starego** snapshotu i nie stosować GET A po nawigacji na B (`SPEC-FRONTEND.md` F-5 / F-5a).

**Artefakty:**

- Zmiana: `apps/frontend/src/modules/runs/components/run-details-view.tsx`
- Zmiana: `apps/frontend/src/app/(app)/runs/[runId]/page.tsx` (`key={runId}` — remount klienta)

#### Refaktor — `run-details-view.tsx`

**Teraz (KROK 4):** `load` + `useCallback` bez anulowania; `live` z samego `isLiveRunStatus(view.snapshot.status)`; błąd walidacji `runId` przez `setView`; `onStatus` / `onLog` bez straży `snapshot.runId`.
...
`runIdRef.current` tylko w efekcie (nie w renderze). Szkielet przy `waitingForSnapshot` (stan z innego id) zamiast synchronicznego `setView({ status: 'loading' })` w efekcie. Niepoprawny `runId`: envelope w renderze, bez GET.

#### Refaktor — `page.tsx` (KROK 4)

**Teraz:** `<RunDetailsView runIdParam={runId} />`.

**Zamień na:** `<RunDetailsView key={runId} runIdParam={runId} />`.

**DoD (krok):** SSE tylko gdy `snapshot.runId === runId` (własny live); in-flight GET po zmianie id jest ignorowany; stary snapshot nie steruje `live`; `key={runId}` na stronie. DoD KROK 4 bez zmian.
