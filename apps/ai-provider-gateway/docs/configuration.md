...
| ------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `MASTER_SYSTEM_PROMPT.md` | yes      | Guardrails and mandatory policy layer; missing file or empty content after processing → **fail-fast** at startup. In this instance: when the user asks for JSON, the reply must pass `JSON.parse` (no markdown fences, no text before/after the document). |
| `MAIN_SYSTEM_PROMPT.md`   | no       | Optional deployment layer (e.g. style, format); missing or empty → skipped. In this instance: JSON output rules for pipeline clients (one task even if `user` is split; escape `"` inside strings). |
