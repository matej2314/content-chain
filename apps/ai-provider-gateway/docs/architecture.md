...
| **Cache** (`src/cache`)                                          | Global dynamic module with **two cache layers** for `POST /api/v1/chat` **and** stream (shared store): (1) **Exact cache** — backend registry (`noop` always, `redis` conditionally, write `SET NX`), `ResponseCacheService`, key from hash of `(modelAlias, clientId, messages, system prompt, effective params)`; entry reads validated with `CachedChatResponseSchema`. (2) **Semantic cache** (`src/cache/semantic/`) — `EmbeddingBackend` port (Ollama adapter) + `VectorStore` port (Redis Search KNN adapter, upsert `HSETNX`); `SemanticCacheService` first HASH-looks up trimmed last-user identity (`getByTextIdentity`); on miss embeds the last `role: user` message of a **single-turn** request, queries the nearest-neighbour index with TAG filter `modelAlias` + `clientId` + `embeddingModel` + `systemSignature` + `callParams`, applies similarity threshold. Pipeline: cooldown → alias policy → exact KV → semantic HASH (trim last-user) → embed+KNN → provider → dual-write sync (await exact SET NX and semantic upsert). No semantic→exact promotion; no store on `didFallback`. Semantic-only (`CACHE_ENABLED=false`) is supported. Default similarity 0.85. Vector TTL = `CACHE_TTL`. Multi-turn → skip semantic; fail-open on embedding/Search error; at most one `embed` per request (reuse lookup vector on store; no retry if `embed` already attempted). **`RedisConnectionService`** (`adapters/redis-cache/`) is shared Redis infrastructure (exact cache + rate limit + semantic cache Search); activation: `isRedisRequiredFromEnv()` in `should-include-redis-stack.ts` — Redis is required when exact cache uses `redis`, smart rate limit is on, **or** `SEMANTIC_CACHE_ENABLED=true` (Search). When `SEMANTIC_CACHE_ENABLED=true`, `checks.embeddings` appears in `GET /api/v1/health/ready` (fail-open, does not block `ready`). Embedding model default: `qwen3-embedding:0.6b` (DIM 1024). Store gate: `shouldStoreChatResponse` (`finishReason=stop`, non-empty text, no `toolCalls`); `requestId` is not stored — a hit stamps the current request; `id` comes from the payload. Env: `configuration.md`. |
...
| **Health** (`src/health`)                                        | Liveness (`GET /api/v1/health`) and readiness (`GET /api/v1/health/ready` — `checks.config`, `checks.redis`, `checks.cache`, optionally `checks.embeddings` / `checks.vectorStore`). **`HealthService`**: `evaluateReadiness()`, `publishMetrics()`, warm-up at startup; hook in `PreMetricsScrapeRegistry` (gauges refreshed on `GET /metrics`). **`checks.redis`**: probe of shared Redis infrastructure (`RedisConnectionService.ping()`), only when `isRedisRequiredFromConfig()`; fields `required`, `consumers` (allowed: `cache`, `rate-limit`, `semantic-cache`). **`checks.cache`**: aggregate of **enabled** pipeline layers (exact Redis KV and/or semantic embeddings + vectorStore); `healthy` only when all enabled layers work, otherwise `degraded` (`exact-redis`, `embeddings`, `vectorStore`). Both off → `Cache disabled (noop)`. `degraded` does not block `ready`. **`checks.embeddings`**: embedding service probe, present only when `SEMANTIC_CACHE_ENABLED=true`; fail-open (does not reset the embedding circuit). **`checks.vectorStore`**: Redis Search / index probe (`FT.INFO`), present only when semantic is enabled; fail-open. Configuration validation at **process startup**.                                               |
